...
__global__ void knn_query_cuda_kernel(int m, int nsample, const float *__restrict__ xyz,
...
void knn_query_cuda_launcher(int m, int nsample, const float *xyz, const float *new_xyz,
    ...
    knn_query_cuda_kernel<<<blocks, threads, 0>>>(m, nsample, xyz, new_xyz, offset,
                                                   new_offset, idx, dist2);
}
